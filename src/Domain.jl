...
function ConstantTVDomain(;phase::Z,initialconds::Dict{X,E},constantspecies::Array{X2,1}=Array{String,1}(),
    ...
    krevsnondiff = @fastmath kfsnondiff./getKcs(phase,T,Gs)
    ...
    return ConstantTVDomain(phase,SVector(phase.species[1].index,phase.species[end].index),constspcinds,
        T,V,kfs,krevs,kfsnondiff,krevsnondiff,efficiencyinds,Gs,rxnarray,mu,diffs,jacobian,sensitivity,MVector(false),MVector(0.0),p), y0, p
end
...
@inline function calcthermo(d::ConstantTVDomain{W,Y},y::J,t::Q,p::Q2=DiffEqBase.NullParameters()) where {Q2<:Array{Float64,1},W<:IdealDiluteSolution,Y<:Integer,J<:Array{Float64,1},Q<:Real}
    ...
    elseif nothermochg
        ...
        d.krevsnondiff = @fastmath d.kfsnondiff./getKcs(d.phase,d.T,d.Gs)
...
function jacobiany!(jac::Q,y::U,p::W,t::Z,domain::D,interfaces::Q3,colorvec::Q2=nothing) where {Q3<:AbstractArray,Q2,Q<:AbstractArray,U<:AbstractArray,W,Z<:Real,D<:ConstantPDomain}
    ...
    Cpave = Cvave+R
    dCpavedn = zeros(domain.indexes[2]-domain.indexes[1]+1)
    for i in domain.indexes[1]:domain.indexes[2]
        dCpavedn[i] = cpdivR[i]*R/N - Cpave/N
        jac[domain.indexes[3],i] = -dot(Hs,jac[domain.indexes[1]:domain.indexes[2],i])/(N*Cpave)-dydt[domain.indexes[3]]*(1/N+dCpavedn[i]/Cpave)
        jac[domain.indexes[4],i] = sum(jac[domain.indexes[1]:domain.indexes[2],i])*R*T/P + V/T*jac[domain.indexes[3],i]
    end
    ...
    for inter in interfaces
        if isa(inter,Inlet) && domain == inter.domain
            ...
            for i in domain.indexes[1]:domain.indexes[2]
                dTdt = flow*(inter.H - dot(Hs,ns)/N)/(N*Cpave)
                ddnidTdt = flow*(-Hs[i]/N + dot(Hs,ns)/N^2)/(N*Cpave)-dTdt*(1/N+dCpavedn[i]/Cpave)
                jac[domain.indexes[3],i] += ddnidTdt
                jac[domain.indexes[4],i] += V/T*ddnidTdt
            end
        elseif isa(inter,Outlet) && domain == inter.domain
            flow = inter.F(t)
            jac[domain.indexes[1]:domain.indexes[2],domain.indexes[1]:domain.indexes[2]] .-= flow/N*Matrix(1.0I,domain.indexes[2]-domain.indexes[1]+1,domain.indexes[2]-domain.indexes[1]+1)
    ...
    jacobiany_therm!(jac,y,p,t,domain,interfaces,domain.indexes[3],T,nothing)
    jacobiany_therm!(jac,y,p,t,domain,interfaces,domain.indexes[4],V,nothing)
...
function jacobiany!(jac::Q,y::U,p::W,t::Z,domain::D,interfaces::Q3,colorvec::Q2=nothing) where {Q3<:AbstractArray,Q2,Q<:AbstractArray,U<:AbstractArray,W,Z<:Real,D<:ParametrizedTPDomain}
    ...
    jacobiany_therm!(jac,y,p,t,domain,interfaces,domain.indexes[3],V,nothing)
...
function jacobiany!(jac::Q,y::U,p::W,t::Z,domain::D,interfaces::Q3,colorvec::Q2=nothing) where {Q3<:AbstractArray,Q2,Q<:AbstractArray,U<:AbstractArray,W,Z<:Real,D<:AbstractDomain}
    ns,cs,T,P,V,C,N,mu,kfs,krevs,Hs,Us,Gs,diffs,Cvave = calcthermo(domain,y,t,p)
    jacobiany_ns!(jac,y,p,t,domain,[],cs,kfs,krevs,nothing)
